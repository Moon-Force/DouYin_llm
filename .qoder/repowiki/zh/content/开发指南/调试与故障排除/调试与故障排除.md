...
- [docs/superpowers/specs/2026-04-15-comment-processing-status-design.md](file://docs/superpowers/specs/2026-04-15-comment-processing-status-design.md)
- [docs/superpowers/plans/2026-04-15-comment-processing-status.md](file://docs/superpowers/plans/2026-04-15-comment-processing-status.md)
...
## 简介
本指南面向DouYin_llm项目的调试与故障排除，覆盖启动问题、连接问题、性能问题、日志分析与调试工具使用，并提供常见错误代码与解决方案参考。随着功能演进，现已新增内存管道验证器和评论处理状态诊断两大自动化工具，替代原有手动验证方式，显著提升问题定位效率。
...
- **新增** 评论处理状态：实时展示每条评论的处理轨迹，包括落库、记忆抽取、召回和提词生成状态。
- **新增** 内存管道验证器：自动化验证记忆抽取、持久化和召回全流程。
...
- CORS：允许任意来源，便于本地联调。
- **新增** 评论处理状态：在事件处理过程中实时构建processing_status，包含落库、记忆抽取、召回和提词生成的完整轨迹。
...
Run --> ProcessEvent["处理事件<br/>构建processing_status"]
ProcessEvent --> PushStream["推送SSE/WebSocket流"]
PushStream --> Shutdown{"应用关闭?"}
Shutdown --> |否| Run
...
- **新增** 回忆元数据：在构建上下文时返回recalled_memory_ids，供processing_status记录召回状态。

```mermaid
flowchart TD
Evt["事件"] --> Type{"事件类型/关键词"}
Type --> |礼物/关注| Heu["启发式生成"]
Type --> |评论| Ctx["构建上下文<br/>返回召回ID"]
Ctx --> Gen["OpenAI兼容请求"]
Gen --> Ok{"成功?"}
Ok --> |是| Save["记录状态/置信度"]
Ok --> |否| Fallback["回退启发式"]
...
### 评论处理状态诊断
**新增** 评论处理状态功能为每条评论提供完整的处理轨迹，前端可直接查看处理结果。

- 状态字段：received、persisted、memory_extraction_attempted、memory_saved、memory_recall_attempted、memory_recalled、suggestion_generated等。
- 实时展示：在SSE/WebSocket流中直接附加processing_status，无需额外查询。
- 前端渲染：评论事件显示"已落库"、"已保存记忆"、"命中召回"、"已生成提词"等状态标签。

**章节来源**
- [backend/schemas/live.py:81-94](file://backend/schemas/live.py#L81-L94)
- [backend/app.py:73-120](file://backend/app.py#L73-L120)
- [docs/superpowers/specs/2026-04-15-comment-processing-status-design.md:40-71](file://docs/superpowers/specs/2026-04-15-comment-processing-status-design.md#L40-L71)

### 内存管道验证器
**新增** 内存管道验证器提供自动化测试和验证功能，替代原有手动验证方式。

- 内部模式：直接调用后端组件进行单元测试，验证记忆抽取、持久化和召回流程。
- 端到端模式：自动启动后端服务，通过HTTP接口验证完整流程。
- 批量数据集：内置50条确定性测试数据，确保测试结果可重复。
- 逐步验证：每个步骤都有详细的错误信息和失败原因分析。

**章节来源**
- [tests/memory_pipeline_verifier/runner.py:211-291](file://tests/memory_pipeline_verifier/runner.py#L211-L291)
- [tests/memory_pipeline_verifier/runner.py:310-354](file://tests/memory_pipeline_verifier/runner.py#L310-L354)
- [tests/memory_pipeline_verifier/datasets.py:38-66](file://tests/memory_pipeline_verifier/datasets.py#L38-L66)
- [docs/superpowers/specs/2026-04-14-memory-pipeline-verifier-design.md:141-152](file://docs/superpowers/specs/2026-04-14-memory-pipeline-verifier-design.md#L141-L152)

## 依赖分析
- Python运行时：FastAPI、Uvicorn、WebSocket客户端、Redis、Chroma。
- 前端：Vue 3、Pinia、Vite、TailwindCSS。
- 启动脚本：PowerShell脚本负责环境检查与端口占用校验。
- **新增** 验证工具：内存管道验证器依赖后端配置和测试数据集。
...
BAT --> VITE
VM["内存管道验证器<br/>tests/memory_pipeline_verifier/*"] --> Py
VM --> FA
```
...
- 响应延迟
  ...
  - **新增** 评论处理状态：状态构建在事件处理过程中完成，不影响主要业务流程。
- 存储与I/O
...
- WebSocket连接（采集器）
  ...
  - **新增** 采集器工具：使用tool/README.md中的配置和连接方式验证采集器状态。
- 数据库连接（SQLite）
  - 症状：写入失败或查询异常。
...
### 四、内存管道验证器使用指南
**新增** 使用内存管道验证器替代手动验证方式，提供自动化测试和诊断能力。

- 内部模式验证
  ```bash
  python tests/verify_memory_pipeline.py --mode internal --count 50
  ```
  - 验证记忆抽取、持久化和召回的完整流程
  - 使用内置50条测试数据，确保结果可重复
  - 显示每个步骤的详细统计信息
...
- 端到端模式验证
  ...
  ```
  - 自动启动后端服务进行完整流程测试
  - 通过HTTP接口验证事件注入、记忆查询等功能
  - 支持自动后端启动和停止

- 批量数据集使用
  ```bash
  python tests/memory_pipeline_verifier/datasets.py
  ```
  - 导出测试数据集到JSON文件
  - 加载自定义数据集进行验证
  - 支持批量测试和回归测试
...
**章节来源**
- [tests/verify_memory_pipeline.py:13-14](file://tests/verify_memory_pipeline.py#L13-L14)
- [tests/memory_pipeline_verifier/runner.py:379-400](file://tests/memory_pipeline_verifier/runner.py#L379-L400)
- [tests/memory_pipeline_verifier/datasets.py:69-87](file://tests/memory_pipeline_verifier/datasets.py#L69-L87)

### 五、评论处理状态诊断方法
**新增** 使用评论处理状态功能快速定位评论相关问题。

- 前端状态查看
  - 评论事件显示"已落库"、"已保存记忆"、"命中召回"、"已生成提词"状态标签
  - 展开详情查看具体的memory_ids和suggestion_id
  - 区分"未处理"、"处理了但没产出记忆"、"处理了且命中召回"三种状态
...
- 状态字段含义
  - `received`: 事件已被后端接收
  - `persisted`: 事件已成功写入数据库
  - `memory_extraction_attempted`: 已尝试记忆抽取
  - `memory_saved`: 已保存观众记忆
  - `memory_recall_attempted`: 已尝试记忆召回
  - `memory_recalled`: 已命中观众记忆
  - `suggestion_generated`: 已生成提词

**章节来源**
- [tests/test_comment_processing_status.py:29-105](file://tests/test_comment_processing_status.py#L29-L105)
- [backend/schemas/live.py:81-94](file://backend/schemas/live.py#L81-L94)
- [docs/superpowers/specs/2026-04-15-comment-processing-status-design.md:40-71](file://docs/superpowers/specs/2026-04-15-comment-processing-status-design.md#L40-L71)
...
- 后端日志
  ...
  - 参考：接口层与Agent的日志输出位置。
  - **新增** 评论处理状态日志：查看processing_status的构建和传递过程。
- 前端日志
  - 关注：WebSocket连接状态、SSE流接收、组件渲染与Store状态变化。
  - **新增** 状态标签渲染：检查评论处理状态标签的显示效果。
- 采集器日志
  - 关注：原始消息结构、连接状态、错误与重连。
  - **新增** 采集器配置：使用tool/config.yaml验证采集器配置。
...
- 浏览器开发者工具
  ...
  - Console：查看前端错误与状态提示。
  - **新增** 状态标签调试：检查评论处理状态标签的渲染和交互。
- Python调试器
  - 在Agent或Collector关键路径设置断点，逐步跟踪事件流转。
  - **新增** 验证器调试：使用unittest框架调试内存管道验证器。
- 网络抓包工具
  - 抓取LLM API请求与响应，核对URL、头部与负载。
- **新增** 内存管道验证器
  - 使用tests/verify_memory_pipeline.py进行自动化测试
  - 支持内部模式和端到端模式
  - 提供详细的测试报告和错误分析
...
- 运行阶段
  ...
  - **新增** 评论处理状态异常：
    - 状态字段缺失：检查process_event函数中的状态构建逻辑
    - 状态显示错误：验证Agent的generation_metadata返回
    - 前端不显示状态：确认LiveEvent的processing_status字段
...
## 结论
通过分层排查（启动、连接、性能、日志与工具）与组件级定位（配置、采集、记忆、LLM），并结合新增的内存管道验证器和评论处理状态诊断功能，可高效解决DouYin_llm在本地开发与演示场景中的常见问题。自动化验证工具显著提升了问题定位效率，建议在生产化前补充可观测性与告警能力，以进一步提升稳定性与可维护性。
...
- 快速检查清单
  ...
  - **新增** 内存管道验证器运行正常，无步骤失败。
  - **新增** 评论处理状态功能正常，前端显示完整状态标签。