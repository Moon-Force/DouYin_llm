...
## 更新摘要
**所做更改**
- 新增生命周期管理章节，详细说明短期记忆的 TTL 过期机制
- 更新配置参数章节，新增 MEMORY_SHORT_TERM_TTL_HOURS 配置项
- 新增时间衰减与生命周期分离的设计说明
- 更新向量存储章节，说明过期记忆在召回过程中的过滤机制
- 新增应用层生命周期决策流程说明
...
**更新** 新增生命周期管理功能，支持短期记忆的 TTL 过期机制和时间衰减的职责分离。
...
**更新** 新增生命周期管理功能，支持短期记忆的 TTL 过期机制。
...
**更新** 新增生命周期管理，短期记忆支持 TTL 过期，长期记忆支持显式过期字段。
...
**更新** 新增生命周期字段支持，包括 lifecycle_kind 和 expires_at 列，提供过期记忆过滤查询。
...
**更新** 新增过期记忆过滤机制，在相似度检索中自动排除已过期的记忆条目。
...
### 生命周期设计概述
系统引入了最小可行的生命周期管理机制，解决短期记忆长期污染长期记忆池的问题。该设计将过期管理分为两个层面：

- **生命周期管理**：通过 expires_at 字段和 TTL 机制控制记忆的过期时间
- **时间衰减**：通过 halflife 机制对未过期记忆进行权重衰减

### 配置参数
新增 MEMORY_SHORT_TERM_TTL_HOURS 配置参数，默认值为 72 小时：
...
### 生命周期策略
1. **长期记忆**（lifecycle_kind="long_term"）
   - expires_at=0，永不过期
   - 适用于稳定的事实类信息

2. **短期记忆**（lifecycle_kind="short_term"）
   - expires_at=created_at + ttl_hours * 3600 * 1000
   - 适用于临时性、时效性强的信息

### 应用层生命周期决策
在内存提取过程中，系统根据 temporal_scope 自动确定生命周期：
...
**章节来源**
- [config.py:124-126](file://backend/config.py#L124-L126)
- [app.py:175-180](file://backend/app.py#L175-L180)
- [long_term.py:804](file://backend/memory/long_term.py#L804)
- [long_term.py:828](file://backend/memory/long_term.py#L828)
...
- 应用层负责生命周期决策，根据 temporal_scope 和配置参数确定 expires_at。
...
- 生命周期过滤：SQLite 查询中集成过期过滤，避免扫描过期数据，提升查询性能。

**更新** 新增生命周期管理的性能考量，包括过期过滤对查询性能的提升。
...
- 生命周期配置错误：检查 MEMORY_SHORT_TERM_TTL_HOURS 环境变量设置，确认 TTL 单位为小时。
- 过期记忆仍可见：确认查询接口正确传递 now_ms 参数，检查 expires_at 字段是否正确设置。

**更新** 新增生命周期管理相关的故障排除指导。
...
**更新** 新增的生命周期管理功能进一步完善了系统的记忆管理能力，通过 TTL 过期机制和时间衰减的职责分离，有效解决了短期信息长期污染长期记忆池的问题，提升了召回质量和系统稳定性。
...
- 生命周期测试：验证短期记忆的 TTL 过期机制、长期记忆的永不过期特性、过期记忆在查询中的过滤效果。

**更新** 新增生命周期管理相关的使用示例和测试验证。
...
- [test_long_term.py:400-450](file://tests/test_long_term.py#L400-L450)