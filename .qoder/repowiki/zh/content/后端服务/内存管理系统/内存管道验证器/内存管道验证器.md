...
- [tests/fixtures/semantic_recall/hard.json](file://tests/fixtures/semantic_recall/hard.json)
...
内存管道验证器是一个专门设计的开发者自检工具，用于验证抖音直播间的观众记忆提取、持久化和语义召回管道的完整性。该工具提供了三种验证模式：内部模式（internal）、端到端模式（e2e）和语义召回模式（semantic-recall），能够确保从评论提炼观众记忆到向量索引召回的整个链路正常工作。

**更新** 新增语义召回评估功能，支持通过切换数据集直接输出 top1/top3 指标，提供更精确的语义相似度评估能力。该功能专门针对"同一观众存在多条记忆时，改写问法能否召回正确记忆"的真实语义质量评估场景。
...
J[hard.json<br/>困难评测集]
K[blind_100.json<br/>盲测评测集]
end
subgraph "后端组件"
L[backend/services/memory_extractor.py<br/>记忆提取器]
M[backend/memory/long_term.py<br/>长期存储]
N[backend/memory/vector_store.py<br/>向量存储]
O[backend/memory/embedding_service.py<br/>嵌入服务]
P[backend/app.py<br/>API接口]
end
D --> A
E --> A
A --> C
...
**图表来源**
- [tests/memory_pipeline_verifier/runner.py:1-618](file://tests/memory_pipeline_verifier/runner.py#L1-L618)
...
### 1. 主验证器（Runner）
主验证器负责协调整个验证流程，包括模式选择、步骤执行和结果汇总。它实现了内部模式、端到端模式和语义召回模式的完整逻辑，并新增了语义召回评估任务分发功能。

### 2. 数据集生成器（Datasets）
数据集生成器提供了确定性的测试数据，确保验证结果的可重复性和一致性。它包含了预定义的城市、话题和内容模板，并新增了语义召回评测集的数据加载和校验功能。

### 3. 语义召回评估器
**新增** 专门负责语义相似度评估的组件，支持 top1/top3 指标统计、失败案例输出、评测集管理和报告生成。
...
1. **多模式支持**：同时支持内部验证、端到端验证和语义召回评估
2. **任务分发机制**：支持 pipeline、semantic-recall 和 e2e 三种任务模式
3. **模块化设计**：每个组件都有明确的职责边界
4. **可扩展性**：新的验证步骤可以轻松添加
5. **指标统计**：内置 top1/top3 指标统计和失败案例分析
6. **报告生成**：自动生成 Markdown 格式的评估报告
7. **错误处理**：完善的异常处理和恢复机制
...
NormalizeTask --> |e2e| E2EMode[E2E模式]
...
FailureReport --> ReportGen[生成Markdown报告]
ReportGen --> Summary
Summary --> End([结束])
```
...
4. **管道模式（Pipeline）**：**默认** 传统的内存管道验证模式
...
MetricsCalc --> ReportGen[生成报告]
ReportGen --> Summary[结果汇总]
Summary --> End([结束])
```
...
6. **多级难度**：支持 default（基础）、hard（困难）、blind_100（盲测）等不同难度级别
...
6. **难度分级**：支持从基础到困难的不同难度级别

**章节来源**
- [tests/memory_pipeline_verifier/datasets.py:89-119](file://tests/memory_pipeline_verifier/datasets.py#L89-L119)
- [tests/fixtures/semantic_recall/default.json:1-63](file://tests/fixtures/semantic_recall/default.json#L1-L63)
- [tests/fixtures/semantic_recall/hard.json:1-328](file://tests/fixtures/semantic_recall/hard.json#L1-L328)
...
subgraph "报告生成"
M[report_path] --> N[Markdown报告]
O[case_results] --> P[详细结果]
end
```
...
6. **失败分析**：生成失败案例报告
7. **报告生成**：生成 Markdown 格式的评估报告

#### 报告生成功能
**新增** 语义召回评估器支持自动生成评估报告：

1. **报告路径**：根据数据集名称生成对应的报告文件路径
2. **报告格式**：生成 Markdown 格式的详细报告
3. **报告内容**：包含摘要、详细结果和失败案例分析
4. **自动保存**：将报告自动保存到 artifacts 目录
...
9. **报告生成**：**新增** 测试 Markdown 报告生成功能
10. **失败案例分析**：**新增** 测试失败案例输出功能
...
**章节来源**
- [tests/test_verify_memory_pipeline.py:1-541](file://tests/test_verify_memory_pipeline.py#L1-L541)
...
D[tests.memory_pipeline_verifier.runner.build_semantic_recall_report_path]
E[tests.memory_pipeline_verifier.runner.render_semantic_recall_report_markdown]
end
subgraph "后端核心组件"
F[backend.services.memory_extractor]
G[backend.memory.long_term]
H[backend.memory.vector_store]
I[backend.memory.embedding_service]
...
+run_semantic_recall_verification(dataset_path, report_dir) : list[StepResult]
+calculate_metrics(cases, vector) : dict
+generate_failure_report(failures) : None
+build_semantic_recall_report_path(dataset_path, report_dir) : Path
+render_semantic_recall_report_markdown(dataset_path, total_cases, top1_hits, top3_hits, case_results) : str
}
class DatasetGenerator {
...
SemanticRecallEvaluator --> StepResult
SemanticRecallEvaluator --> Path
```
...
1. **批量索引构建**：一次性构建所有候选记忆的向量索引
2. **限制输出数量**：最多输出前5个失败案例，避免大量日志输出
3. **报告生成优化**：使用高效的 Markdown 渲染引擎
4. **内存池管理**：复用嵌入向量计算结果
...
7. **报告检查**：**新增** 检查生成的 Markdown 报告内容
8. **性能监控**：**新增** 监控语义召回评估的执行时间
...
内存管道验证器是一个设计精良的开发者自检工具，它成功地解决了直播间观众记忆管道验证的核心需求。通过提供内部模式、端到端模式、语义召回模式和默认管道模式，它能够满足不同场景下的验证需求。
...
7. **报告生成**：**新增** 自动生成 Markdown 格式的详细评估报告
8. **多难度支持**：**新增** 支持从基础到困难的不同难度级别评测集

### 技术亮点

1. **多模式架构**：内部模式、端到端模式和语义召回模式的灵活切换
2. **任务分发机制**：**新增** 支持 pipeline、semantic-recall 和 e2e 三种任务模式
3. **健壮的错误处理**：完善的异常处理和恢复机制
4. **清晰的输出格式**：详细的步骤报告和总体结果
5. **指标统计功能**：**新增** 内置 top1/top3 指标计算和分析
6. **测试驱动开发**：完整的单元测试覆盖
7. **报告生成系统**：**新增** 自动化的评估报告生成
8. **多级评测支持**：**新增** 支持 default、hard、blind_100 等不同难度级别
...
7. **并发执行**：**新增** 支持多线程并发执行语义召回评估
8. **云端集成**：**新增** 支持云端嵌入服务的语义评估

内存管道验证器为直播间的观众记忆系统提供了一个可靠的验证基础，有助于提高系统的质量和开发效率。新增的语义召回评估功能进一步增强了工具的实用性和精确性，为语义相似度评估提供了标准化的解决方案。通过支持多种难度级别的评测集和自动化的报告生成功能，该工具为语义召回质量的持续改进提供了强有力的技术支撑。