...
## 更新摘要
**变更内容**
- 新增观众记忆纠正功能，支持手动编辑、状态管理、审计日志
- 新增语义召回评估工具，支持top1/top3指标统计
- 增强索引管理功能，新增索引预热和一致性检查
- 改进召回排序算法，增加置顶、手动标记、置信度权重
- 扩展向量操作接口，新增索引重建和同步功能
...
- backend/schemas/live.py：LiveEvent、ViewerMemory等数据模型，新增状态字段
- tests/*：单元测试覆盖向量存储与嵌入服务的关键行为
- docs/superpowers/plans：新功能规划文档，包括语义召回评估和观众记忆纠正

```mermaid
graph TB
subgraph "向量存储层"
VM["VectorMemory<br/>Chroma 集合<br/>索引管理"]
...
## 核心组件
- VectorMemory：Chroma持久化客户端、事件与观众记忆集合、相似度检索与重排序、回退索引、索引管理、状态同步
...
**章节来源**
- [vector_store.py:34-388](file://backend/memory/vector_store.py#L34-L388)
- [embedding_service.py:18-102](file://backend/memory/embedding_service.py#L18-L102)
...
B --> D["LongTermStore(SQLite)<br/>状态管理/审计日志"]
B --> E["VectorMemory(Chroma)<br/>索引管理/状态同步"]
...
- 嵌入与存储策略
  ...
  - 观众记忆集合文档为记忆文本，元数据包含房间ID、观众ID、记忆类型、来源事件ID、置信度、更新时间、召回次数、状态、来源类型、是否置顶
...
- 相似度检索与重排序
  ...
  - 观众记忆重排序键：(reranked_score, updated_at)，reranked_score综合了向量分数、置信度、查询词包含、召回次数、来源类型、是否置顶
...
**章节来源**
- [live.py:29-78](file://backend/schemas/live.py#L29-L78)
- [vector_store.py:149-388](file://backend/memory/vector_store.py#L149-L388)

### 观众记忆纠正功能
- 状态管理
  - 支持active（激活）、invalid（无效）、deleted（删除）三种状态
  - 自动过滤无效状态的记忆参与召回
  - 状态变更时自动记录审计日志

- 权限控制
  - source_kind区分manual（手动）和auto（自动）来源
  - 手动来源的记忆在排序时具有更高优先级
  - 支持置顶功能，置顶的记忆在排序时具有最高优先级
