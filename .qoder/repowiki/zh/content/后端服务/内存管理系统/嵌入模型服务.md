...
## 更新摘要
**变更内容**
- 新增严格模式（EMBEDDING_STRICT）配置和实现
- 增强健康监控，支持语义后端状态检测
- 改进错误处理机制，严格模式下禁止哈希回退
- 添加语义后端可用性检测和错误报告
- 更新测试用例覆盖严格模式行为
...
**更新** 新增严格模式（EMBEDDING_STRICT）配置，支持强制使用真实语义召回，禁止哈希回退，提供更精确的语义后端健康监控。
...
- tests/test_vector_store.py：向量存储单元测试，覆盖严格模式下的回退行为。
- tests/test_rebuild_embeddings.py：重建工具单元测试，覆盖严格模式下的失败行为。
- tests/test_comment_processing_status.py：健康监控测试，验证 /health 接口的语义后端状态。
- requirements.txt：运行依赖清单，包含可选的 chromadb、sentence-transformers 等。
- README.md：项目总体说明与接口概览。
- .env.example：环境变量配置示例，包含严格模式配置。
...
**更新** 新增严格模式配置（EMBEDDING_STRICT），支持强制使用真实语义召回，禁止哈希回退。
...
**更新** 健康监控集成到 /health 接口，提供语义后端状态和原因信息。
...
**更新** 新增严格模式检查，当启用严格模式且发生异常时，抛出 RuntimeError 而不是回退到哈希嵌入。
...
Mode --> |其他| StrictCheck{"严格模式检查"}
StrictCheck --> |启用| StrictError["抛出严格模式错误"]
StrictCheck --> |禁用| Fallback["回退到哈希嵌入函数"]
...
**更新** 新增严格模式检查，当启用严格模式且语义后端不可用时，抛出 RuntimeError 而不是回退到内存回退索引。
...
+_embed_local(texts)
+_embed_cloud(texts)
+_strict_mode_enabled() bool
+_raise_strict_mode_error(exc)
}
...
+semantic_backend_ready() bool
+semantic_backend_reason() string
...
**更新** 严格模式下，当嵌入生成失败时，直接抛出 RuntimeError 而不是写入伪嵌入结果。
...
- 严格模式错误处理
  - 启用严格模式时，任何异常都抛出 RuntimeError，不进行回退。
- LLM 侧错误处理参考
  - 虽非嵌入服务，但 LLM 侧同样采用统一的错误分类与状态标记，可借鉴其错误处理模式。

**更新** 严格模式下，错误处理更加严格，不允许任何形式的回退。
...
### 严格模式（EMBEDDING_STRICT）
严格模式是一个重要的配置选项，用于强制系统使用真实的语义召回，而不是回退到哈希嵌入或其他替代方案。

#### 配置与启用
- 环境变量：`EMBEDDING_STRICT=true`
- 配置解析：在 Settings 类中解析为布尔值
- 默认值：false（非严格模式）

#### 行为差异
在严格模式下，系统的行为与非严格模式有显著差异：

1. **嵌入生成失败时**：
   - 非严格模式：回退到哈希嵌入函数
   - 严格模式：抛出 RuntimeError，不进行回退
...
### 健康监控与状态报告
系统提供了完整的健康监控机制，通过 `/health` 接口暴露语义后端的状态信息。

#### /health 接口扩展
新的 `/health` 接口返回以下字段：

1. **embedding_strict**：当前是否启用严格模式
2. **semantic_backend_ready**：语义后端是否可用
3. **semantic_backend_reason**：语义后端不可用的原因

#### 语义后端状态检测
VectorMemory 提供了专门的方法来检测和报告语义后端状态：

1. **semantic_backend_ready()**：检查 Chroma 客户端和集合是否存在
2. **semantic_backend_reason()**：返回具体的不可用原因
3. **_ensure_semantic_backend()**：在严格模式下检查后端状态

#### 健康监控的重要性
健康监控对于区分不同类型的失败场景至关重要：

- **语义召回返回空结果**：可能是由于没有匹配的记忆，这是正常现象
- **语义后端不可用**：可能是 Chroma 服务故障、网络问题或配置错误

通过健康监控，操作员可以立即识别问题的根本原因，采取相应的修复措施。

**章节来源**
- [backend/config.py:64-76](file://backend/config.py#L64-L76)
- [backend/app.py:241-252](file://backend/app.py#L241-L252)
- [backend/memory/vector_store.py:86-105](file://backend/memory/vector_store.py#L86-L105)
- [tests/test_embedding_service.py:81-100](file://tests/test_embedding_service.py#L81-L100)
- [tests/test_vector_store.py:94-112](file://tests/test_vector_store.py#L94-L112)
- [tests/test_rebuild_embeddings.py:262-318](file://tests/test_rebuild_embeddings.py#L262-L318)
...
- 严格模式性能影响
  - 严格模式下，系统不会进行任何回退，可能导致更高的失败率。
  - 建议在生产环境中配合健康监控使用，及时发现和解决问题。

**更新** 严格模式可能影响系统整体的成功率，需要配合健康监控使用。
...
- 严格模式问题
  - 检查 EMBEDDING_STRICT 配置是否正确设置。
  - 严格模式下任何异常都会导致失败，需要排查根本原因。
- 健康监控
  - 访问 `/health` 接口查看语义后端状态。
  - 根据 semantic_backend_reason 字段判断具体问题原因。
- 单元测试验证
  - 使用测试用例验证云端/本地/回退路径，确保预期行为符合预期。

**更新** 新增严格模式和健康监控的故障排查指导。
...
**更新** 新增的严格模式和健康监控机制进一步增强了系统的可靠性，能够精确地区分不同类型的失败场景，为运维人员提供更好的问题诊断能力。
...
- 健康监控
  - 访问 GET /health 接口获取系统健康状态，包括 embedding_strict、semantic_backend_ready 和 semantic_backend_reason。
...
- 在需要严格语义召回的场景下，启用 EMBEDDING_STRICT=true 并配合健康监控使用。
- 定期检查 /health 接口，监控语义后端的可用性和健康状态。

**更新** 新增严格模式和健康监控的最佳实践建议。
...
**章节来源**
- [.env.example:21-29](file://.env.example#L21-L29)
- [README.md:162-182](file://README.md#L162-L182)