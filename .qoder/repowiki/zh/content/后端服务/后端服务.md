...
**更新** 新增了LLM内存提取器和本地内存模型两个新组件，扩展了后端服务的架构能力。
...
- 服务层：
  ...
  - **LLM内存提取器**：backend/services/llm_memory_extractor.py
  - **本地内存模型**：backend/services/local_memory_model.py
- 内存层：
...
LMX["LLM内存提取器<br/>backend/services/llm_memory_extractor.py"]
LMM["本地内存模型<br/>backend/services/local_memory_model.py"]
...
**图表来源**
- [backend/app.py:24-26](file://backend/app.py#L24-L26)
- [backend/config.py:121-135](file://backend/config.py#L121-L135)
- [backend/services/llm_memory_extractor.py:35-38](file://backend/services/llm_memory_extractor.py#L35-L38)
- [backend/services/local_memory_model.py:9-16](file://backend/services/local_memory_model.py#L9-L16)
...
- 配置中心
  - 从环境变量与 .env 加载，提供采集、后端、LLM、嵌入、向量检索、**内存提取器**等参数
  - 提供 LLM 基座地址与模型名解析逻辑
- 数据模型
...
- **新增内存提取器系统**
  - **LLMBackedViewerMemoryExtractor**：基于本地或云端LLM的智能记忆抽取
  - **LocalMemoryExtractionModel**：本地推理运行时封装，支持模型下载、缓存和推理
...
participant MEX as "ViewerMemoryExtractor"
participant LMX as "LLMBackedViewerMemoryExtractor"
participant LMM as "LocalMemoryExtractionModel"
...
API->>MEX : "extract(event)"
MEX->>LMX : "extract(event)"
LMX->>LMM : "infer_json()"
LMM-->>LMX : "JSON响应"
LMX-->>MEX : "规范化记忆"
MEX-->>API : "记忆候选"
API->>LTS : "save_viewer_memory()"
API->>VEC : "sync_memory()"
...
- [backend/services/memory_extractor.py:129-142](file://backend/services/memory_extractor.py#L129-L142)
...
SaveMem --> SyncMem["VectorMemory.sync_memory()"]
...
- **复合记忆抽取器**：结合LLM和启发式规则
- **LLM提取器**：基于本地或云端LLM的智能记忆抽取
- **规则提取器**：传统启发式规则，过滤低信号评论
- **回退机制**：LLM失败时自动回退到规则提取器

**章节来源**
- [backend/services/memory_extractor.py:123-142](file://backend/services/memory_extractor.py#L123-L142)

### **新增** LLM内存提取器（LLMBackedViewerMemoryExtractor）
- **LLM驱动的记忆抽取**：专门针对直播场景设计的智能记忆抽取
- **严格的数据验证**：确保输出符合预定义格式和约束
- **情感分析**：识别正向、负向、中性情感倾向
- **时间范围限定**：仅抽取长期记忆（long_term）
- **置信度映射**：将LLM输出的置信度标准化为系统使用的数值
...
+_configured_model_file_path() Path
+_is_valid_model_file(path) bool
+resolve_model_path() Path
+_resolve_llama_factory() class
+_runtime_instance() object
+infer_json(system_prompt, user_prompt) str
}
LLMBackedViewerMemoryExtractor --> LocalMemoryExtractionModel : "使用本地推理运行时"
```
...
### **新增** 本地内存模型（LocalMemoryExtractionModel）
- **懒加载推理运行时**：按需初始化本地LLM推理实例
- **模型管理**：自动检测、下载和缓存本地模型文件
- **配置灵活**：支持多种模型路径和下载源
- **线程安全**：支持多线程推理调用
- **错误处理**：完善的异常处理和回退机制
...
## 依赖关系分析
- 外部依赖
  - FastAPI、Uvicorn、websocket-client、redis、chromadb、**llama-cpp-python**
- 内部模块耦合
  - app.py 作为中枢，依赖各服务与内存模块
  - agent 依赖 vector_store 与 long_term
  - vector_store 依赖 embedding_service
  - collector 与 app 通过事件循环通信
  - **memory_extractor 依赖 llm_memory_extractor 和 local_memory_model**
...
APP --> MEX["services/memory_extractor.py"]
MEX --> LMX["services/llm_memory_extractor.py"]
LMX --> LMM["services/local_memory_model.py"]
AGN --> VEC
AGN --> LTS
...
- **新增内存提取器性能**
  - **本地推理**：使用llama-cpp-python进行本地推理，减少网络延迟
  - **模型缓存**：首次加载后缓存推理实例，避免重复初始化
  - **线程池**：支持多线程推理调用，提高并发处理能力
  - **配置优化**：可调整上下文大小、线程数、最大token等参数
...
- **新增内存提取器问题**
  - **模型文件缺失**：检查MEMORY_EXTRACTOR_MODEL_PATH配置是否正确
  - **LLM推理失败**：确认llama-cpp-python已安装，检查模型文件完整性
  - **内存不足**：调整MEMORY_EXTRACTOR_CONTEXT_SIZE和MEMORY_EXTRACTOR_THREADS参数
  - **推理超时**：增加MEMORY_EXTRACTOR_TIMEOUT_SECONDS或减少上下文大小

**章节来源**
- [USAGE.md:198-256](file://USAGE.md#L198-L256)

## 结论
该后端服务以 FastAPI 为核心，围绕事件采集、短期/长期/向量内存与智能提词引擎构建了完整的直播提词工作栈。通过可选依赖与降级策略，既能在本地快速运行，也能在生产环境中扩展。**新增的LLM内存提取器和本地内存模型组件进一步增强了系统的智能化水平，提供了更准确的观众记忆抽取能力。** 建议后续完善鉴权、多房间调度、可观测性与模型管理策略，以支撑更复杂的直播运营场景。
...
- **新增配置项**
  - **MEMORY_EXTRACTOR_ENABLED**：启用/禁用内存提取器功能
  - **MEMORY_EXTRACTOR_MODE**：内存提取器模式（local/remote）
  - **MEMORY_EXTRACTOR_MODEL_PATH**：模型文件路径
  - **MEMORY_EXTRACTOR_MODEL_URL**：模型下载URL
  - **MEMORY_EXTRACTOR_MODEL_FILENAME**：模型文件名
  - **MEMORY_EXTRACTOR_CONTEXT_SIZE**：上下文大小
  - **MEMORY_EXTRACTOR_MAX_TOKENS**：最大生成token数
  - **MEMORY_EXTRACTOR_TIMEOUT_SECONDS**：推理超时时间
  - **MEMORY_EXTRACTOR_THREADS**：推理线程数
