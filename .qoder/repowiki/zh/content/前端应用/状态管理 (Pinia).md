...
DouYin_llm项目采用Pinia作为状态管理库，实现了完整的直播场景状态管理解决方案。该系统通过单一的`useLiveStore`存储管理直播房间、SSE连接、事件流、提词建议、LLM配置和观众工作台等核心功能。
