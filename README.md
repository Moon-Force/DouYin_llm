...
2. **给观众记忆增加人工修正能力**：这是当前最值得优先补的一层产品能力。现在记忆主要靠自动抽取，虽然已经能参与真实语义召回，但如果缺少确认、删除、纠错、置顶这类运营入口，错误记忆会长期污染后续召回结果，最终直接影响系统可信度。
3. **减少 `backend.app` 的导入副作用**：当前模块导入时会初始化较多全局资源，测试隔离和启动时序都还有优化空间。
4. **增强失败恢复与运维提示**：现在前端已经能看到“为什么没生成提词”，下一步更值得补的是连续失败告警、恢复建议和排障路径，而不只是状态展示。
5. **优化前端展示层级**：事件流已经能看评论处理状态，但仍偏调试面板风格，后续可以继续强化产品化表达。
...
4. **观众记忆仍以自动抽取为主**：`ViewerWorkbench` 目前更适合查看与记笔记，还没有完整的记忆修正工作流；这意味着一旦自动抽取出错，错误记忆仍可能持续参与语义召回，长期影响召回可信度。
